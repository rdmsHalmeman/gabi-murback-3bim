...
const dbConfig = {
  ...
  user: 'radames', // Usuário do PostgreSQL
  password: 'Lageado001.',

  database: 'gabi-murback-milkmoo',
...
const searchPath = 'peer, public'; // Prioriza schema existente 'peer' e fallback para 'public'
...
const testConnection = async () => {
  try {
    ...
    // Definir o search_path para os schemas disponíveis
...
const query = async (text, params) => {
  const client = await pool.connect();
  try {
    // Definir o search_path para schemas disponíveis no seu banco
    await client.query('SET search_path TO ' + searchPath);
