...
exports.criarProduto = async (req, res) => {
  try {
    const { id_produto, nome_produto, quantidade_estoque, preco_unitario } = req.body;
    ...
    const idValue = id_produto != null && String(id_produto).trim() !== '' ? Number(String(id_produto).trim()) : null;
    if (idValue != null && !Number.isInteger(idValue)) {
      return res.status(400).json({ error: 'id_produto deve ser um inteiro válido' });
    }

    const qtyStr = quantidade_estoque != null ? String(quantidade_estoque).replace(',', '.').trim() : null;
    const qtyValue = qtyStr != null && qtyStr !== '' ? Number(qtyStr) : null;
    if (qtyValue != null && !Number.isInteger(qtyValue)) {
      return res.status(400).json({ error: 'quantidade_estoque deve ser um inteiro válido' });
    }

    const priceStr = preco_unitario != null ? String(preco_unitario).replace(',', '.').trim() : null;
    const priceValue = priceStr != null && priceStr !== '' ? Number(priceStr) : null;
    ...
    let params = [];
    if (idValue == null) {
      // id auto-increment (caso a tabela use sequence/serial)
      sql = 'INSERT INTO produto (nome_produto, quantidade_estoque, preco_unitario) VALUES ($1, $2, $3) RETURNING *';
      params = [String(nome_produto).trim(), qtyValue, priceValue];
    } else {
      sql = 'INSERT INTO produto (id_produto, nome_produto, quantidade_estoque, preco_unitario) VALUES ($1, $2, $3, $4) RETURNING *';
      params = [idValue, String(nome_produto).trim(), qtyValue, priceValue];
    }
